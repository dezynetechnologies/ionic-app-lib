...
Start.fetchWrapper = function fetchWrapper(options) {
  ...
  var repoUrl = 'https://github.com/dezynetechnologies/' + WRAPPER_REPO_NAME + '/archive/master.zip';
...
Start.fetchIonicStarter = function(options) {
  ...
  var repoUrl = ['https://github.com/dezynetechnologies/', repoName].join('')
...
Start.promptForOverwrite = function promptForOverwrite(targetPath) {
  ...
  prompt.get({properties: promptProperties}, function (err, promptResult) {
    events.emit('verbose', 'Prompt results from Start.promptForOverwrite', err, promptResult);
